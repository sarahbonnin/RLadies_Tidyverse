...
author: "R ladies Barcelona - 2019-11-14 - Sarah Bonnin"
...
* All packages share **common practices**: philosophy, grammar, and data structure.
...
- **Dimensions** of tibble readily available.
- Information about **data types** as you display the tibble.
...
What is wrong here ? -- **2 variables in the third column !**
...
* **Separate** column **rate** into 2: **cases** and **population**
* **Unite** columns **century** and **year** into column **year**: **1999** instead of **19** and **99**. *(check ?unite)*.
...
Re-assign to a new - or the same - data frame/tibble as with the regular base R **assignment operator: <-**
...
Get average length of mandates + total count of entries
...
You can combine **summarise()** with **group_by()** to get the average length of mandates **per political party**
...
mutate(table1,
       percent_cases= (cases / population)*100) %>%
  ggplot(aes(x=percent_cases, col=country, fill=country)) + geom_bar()
